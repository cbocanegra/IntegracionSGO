...
BEGIN
	...
	IF VAL=0 THEN
		...
		IF PARAM_CCMPN='AM' THEN		
				...
				SET STRSQL01=
					'SELECT ' CONCAT  
					'A3.NPROPR, A1.CCMPN, A1.NWRRNT, A1.NOPRCN, A1.NCRTDP, A1.CADNA, A1.NANDCL, A1.NPDDUA, A1.FPRMVN, A1.FVNCMX, A1.SSTCOP, A1.SESTRG,' CONCAT
	...
	CALL INTEGRASGO.SP_INTSGO_GEN_ILOG('INTEGRASGO.SP_INTSGO_WAR_CONSULTAR_WAR_GEN',
	pParametros,
	pRespuesta
	);	
...
BEGIN
	...
	IF PARAM_STPQRY <> '' AND PARAM_STPQRY NOT IN ('C','B') THEN
		SET VAL=VAL+1;
		SET PARAM_PROCDESC = PARAM_PROCDESC || 'Flag tipo Búsqueda  no válido,';	
	END IF;
	
	IF (PARAM_STPQRY <> '' AND PARAM_CODQRY = 0) OR (IFNULL(PARAM_STPQRY,'')='' AND PARAM_CODQRY>0) THEN
		SET VAL=VAL+1;
		SET PARAM_PROCDESC = PARAM_PROCDESC || 'Código de búsqueda no válido,';	
	END IF;
	...
	IF VAL=0 THEN
		...
		IF PARAM_CCMPN='AM' THEN		
				...
				IF PARAM_STPQRY='C' THEN
					SET STRSQL01=STRSQL01 CONCAT ' AND CCLNT=' CONCAT CAST(IFNULL(PARAM_CODQRY,0) AS VARCHAR(100));
				ELSEIF PARAM_STPQRY='B' THEN
					SET STRSQL01=STRSQL01 CONCAT ' AND CFNNC=' CONCAT CAST(IFNULL(PARAM_CODQRY,0) AS VARCHAR(100));				
				END IF;
	...
	IF VAL=0 THEN
		...
		IF PARAM_CCMPN='AM' THEN		
				...
				SET STRSQL01=
					...
					'WHERE NOPRCN=' CONCAT CAST(IFNULL(PARAM_NOPRCN,0) AS VARCHAR(100));
