...
(
	...
	IN PARAM_CANTIDAD	NUMERIC (15,5) ,
	IN PARAM_UNIDAD	CHAR (3) ,
	IN PARAM_SCNINP CHAR(1),--EN ESPERA DE CONFIRMACION
	INOUT PARAM_PROCSTATUS INT,
	INOUT PARAM_PROCDESC VARCHAR(2000)	
)
BEGIN
...
--	END IF;	
	...
	IF VAL=0 THEN	
		IF PARAM_CCMPN='AM' THEN
			--IF EXISTS (SELECT NPROPR FROM DC@ALMAPER.ZZWW10 WHERE NPROPR=PARAM_NPROPR) AND 
			--   EXISTS (SELECT NPROPR FROM DC@ALMAPER.ZZWW28 WHERE NPROPR=PARAM_NPROPR) AND 
			--   EXISTS (SELECT NPROPR FROM DC@ALMAPER.ZZWW24 WHERE NPROPR=PARAM_NPROPR) THEN
			--GUARDA
				UPDATE DC@ALMAPER.ZZWW10
			...
			--ELSE
			--	SET PARAM_PROCSTATUS=1;
			--	SET PARAM_PROCDESC = PARAM_PROCDESC || 'REGISTRO NO EXISTE PARA ACTUALIZAR';				
			--END IF;
			
		ELSEIF PARAM_CCMPN='LZ' THEN
			--IF EXISTS (SELECT NPROPR FROM DC@RNSLIB.ZZWW10 WHERE NPROPR=PARAM_NPROPR) AND 
			--   EXISTS (SELECT NPROPR FROM DC@RNSLIB.ZZWW28 WHERE NPROPR=PARAM_NPROPR) AND 
			--   EXISTS (SELECT NPROPR FROM DC@RNSLIB.ZZWW24 WHERE NPROPR=PARAM_NPROPR) THEN
			--GUARDA
				UPDATE DC@RNSLIB.ZZWW10
				SET SESTRG='A', FULTAC=DC@RNSLIB.FECHA(), HULTAC=DC@RNSLIB.HORA(), CULUSA=PARAM_CULUSA
				WHERE NPROPR=PARAM_NPROPR;
...
--	END IF;	
	...
	IF VAL=0 THEN
		...
		IF PARAM_CCMPN='AM' THEN
			--IF EXISTS (SELECT NOPRCN, NPROPR FROM DC@ALMAPER.ZZWW10 WHERE NOPRCN=PARAM_NPROPR AND NOPRCN=0) THEN
				UPDATE DC@ALMAPER.ZZWW10
				SET SSTCOP='',FULTAC=DC@RNSLIB.FECHA(), HULTAC=DC@RNSLIB.HORA(), CULUSA=PARAM_CULUSA
				WHERE NOPRCN=PARAM_NOPRCN AND NOPRRL=0;
			...
			--END IF;
		ELSEIF PARAM_CCMPN='LZ' THEN
			--IF EXISTS (SELECT NOPRCN, NPROPR FROM DC@RNSLIB.ZZWW10 WHERE NOPRCN=PARAM_NPROPR AND NOPRCN=0) THEN
				UPDATE DC@RNSLIB.ZZWW10
				SET SSTCOP='',FULTAC=DC@RNSLIB.FECHA(), HULTAC=DC@RNSLIB.HORA(), CULUSA=PARAM_CULUSA
				WHERE NOPRCN=PARAM_NOPRCN AND NOPRRL=0;
...
--	END IF;		
	...
	IF VAL=0 THEN
		...
		IF PARAM_CCMPN='AM' THEN
			...
			--IF EXISTS (SELECT * FROM DC@ALMAPER.ZZWT89 WHERE NPROPR=PARAM_NPROPR AND STPENT='C') AND EXISTS (SELECT * FROM DC@ALMAPER.ZZWT90 WHERE NPROPR=PARAM_NPROPR) THEN
				DELETE FROM DC@ALMAPER.ZZWT89
				WHERE NPROPR=PARAM_NPROPR AND STPENT='C' AND NOPRCN=PARAM_NOPRCN;
		...
		ELSEIF PARAM_CCMPN='LZ' THEN
			...
			--IF EXISTS (SELECT * FROM DC@RNSLIB.ZZWT89 WHERE NPROPR=PARAM_NPROPR AND STPENT='C') AND EXISTS (SELECT * FROM DC@RNSLIB.ZZWT90 WHERE NPROPR=PARAM_NPROPR) THEN
	...
	IF VAL=0 THEN
		...
		IF PARAM_CCMPN='AM' THEN				
			...
			IF STPQRY <>'P' THEN
				...
				END IF;
			ELSE
				SET STRSQL01=STRSQL01 CONCAT ' AND A1.NPROPR =' CONCAT CAST(IFNULL(PARAM_CODQRY,0) AS VARCHAR(100));
